...
type Recon = { id: string; routeNo: string; edoId?: string; site?: string; sourceName?: string; reconDate: string; premierCrateTotalOutstanding?: number; premierDollyTotalOutstanding?: number; isCompletedReconDate?: boolean; crate: { allowance: number; previousOutstanding: number; issuedToday: number; returnedToday: number; currentOutstanding: number; signedClosingPosition?: number }; dolly: { previousOutstanding: number; issuedToday: number; returnedToday: number; currentOutstanding: number; signedClosingPosition?: number } };
...
export default function CrateControlPage() {
    ...
    const isAll = user?.userType === 'taskraft' && selectedRoute === ALL, routes = useMemo(() => [...new Set(recons.map(r => r.routeNo))].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })), [recons]), latestCompletedRecon = recons.find(r => r.isCompletedReconDate === true) ?? null, latestDate = latestCompletedRecon?.reconDate || recons[0]?.reconDate || '', routeRecons = useMemo(() => recons.filter(r => selectedRoute === ALL || r.routeNo === selectedRoute).sort((a, b) => b.reconDate.localeCompare(a.reconDate)), [recons, selectedRoute]), current = selectedRoute === ALL ? null : (routeRecons.find(r => r.isCompletedReconDate === true) ?? routeRecons[0] ?? null);
    ...
    const exportHistory = useMemo(
        () => recons
            .filter(r => (isAll || r.routeNo === selectedRoute))
            .filter(r => (!fromDate || r.reconDate >= fromDate) && (!toDate || r.reconDate <= toDate))
            .sort((a, b) => a.reconDate.localeCompare(b.reconDate) || a.routeNo.localeCompare(b.routeNo, undefined, { numeric: true })),
        [recons, isAll, selectedRoute, fromDate, toDate]
    );
    const latestAll = useMemo(() => isAll ? recons.filter(r => r.reconDate === latestDate) : [], [isAll, recons, latestDate]), totals = useMemo(() => { const source = latestAll[0], crateOutstanding = Number(source?.premierCrateTotalOutstanding ?? 0), dollyOutstanding = Number(source?.premierDollyTotalOutstanding ?? 0), allow = latestAll.reduce((sum, r) => sum + r.crate.allowance, 0), applicableRate = rateFor(latestDate); return { prev: crateOutstanding, allow, short: crateOutstanding, exposure: crateOutstanding * Number(applicableRate?.crateCost ?? 0), current: crateOutstanding, dollyPrev: dollyOutstanding, dollyCurrent: dollyOutstanding } }, [latestAll, rates, latestDate]);
    const rate = current ? rateFor(current.reconDate) : null, crate = current?.crate ?? { allowance: 0, previousOutstanding: 0, issuedToday: 0, returnedToday: 0, currentOutstanding: 0 }, dolly = current?.dolly ?? { previousOutstanding: 0, issuedToday: 0, returnedToday: 0, currentOutstanding: 0 }, crateCost = Number(rate?.crateCost ?? 0), dollyCost = Number(rate?.dollyCost ?? 0), short = Math.max(0, crate.currentOutstanding), exposure = short * crateCost, within = short === 0, util = crate.allowance ? crate.currentOutstanding / crate.allowance * 100 : 0, bar = Math.min(100, util);
    function downloadHistory() {
        ...
        doc.text(`Period: ${prettyDate(fromDate)} to ${prettyDate(toDate)}  |  Records: ${exportHistory.length}`, 14, 29);

        const headers = ['Date', 'Route', 'C Prev', 'C Iss', 'C Rec', 'C Net', 'Allow', 'Short', 'Exposure', 'C Current', 'D Prev', 'D Iss', 'D Rec', 'D Net', 'D Current', 'D Value'];
        const widths = [22, 18, 15, 15, 15, 15, 15, 15, 27, 18, 15, 15, 15, 15, 18, 27];
        const xStart = 14;
        let y = 40;
        ...
        for (const r of exportHistory) {
            ...
            const cratesShort = Math.max(0, r.crate.currentOutstanding);
            const chargeExposure = cratesShort * replacementCost;

            const dollyReplacementCost = Number(applicableRate?.dollyCost ?? 0);
            const crateNet = r.crate.issuedToday - r.crate.returnedToday;
            const dollyNet = r.dolly.issuedToday - r.dolly.returnedToday;
            const dollyOutstandingValue = r.dolly.currentOutstanding * dollyReplacementCost;

            const row = [
                prettyDate(r.reconDate), r.routeNo,
                String(r.crate.previousOutstanding), String(r.crate.issuedToday), String(r.crate.returnedToday), String(crateNet),
                String(r.crate.allowance), String(cratesShort), money(chargeExposure), String(r.crate.currentOutstanding),
                String(r.dolly.previousOutstanding), String(r.dolly.issuedToday), String(r.dolly.returnedToday), String(dollyNet),
                String(r.dolly.currentOutstanding), money(dollyOutstandingValue),
            ];
        ...
        doc.setTextColor(90);
        doc.text(
            'Crate exposure is indicative risk only. Dollies have no allowance; Outstanding Dolly Value is current outstanding x effective replacement cost.',
            14,
            y
        );
