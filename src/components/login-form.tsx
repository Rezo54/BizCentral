  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Login Failed',
        description: err?.code ? getFirebaseErrorMessage(err) : err?.message || 'Login failed',
      });
    } finally {
      setIsLoading(false);
    }
      ...
      {showReset && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-sm space-y-4 shadow-lg">
            ...
            <input type="email" placeholder="Email address" className="w-full border px-3 py-2 rounded" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} />
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowReset(false)} className="px-3 py-2 text-sm text-gray-600">Cancel</button>
              <button onClick={handlePasswordReset} disabled={resetLoading} className="px-4 py-2 bg-blue-600 text-white rounded">{resetLoading ? 'Sending...' : 'Send Reset'}</button>
            </div>
          </div>
        </div>
      )}
